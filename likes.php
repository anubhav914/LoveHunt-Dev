...
if ($token) {
	...
	$query = "SELECT friend_id from user_choices where choice = 'likes' AND uid = $my_id";
	$resource = pg_query($conn, $query);

	$liked_friends = pg_fetch_all_columns($resource, 0);
	$key = 0;
	$friends = array();
	foreach($liked_friends as $k => $value)
	{
...
?>
					...
					<div class="options">
						<div id='<?php echo "like$counter"; ?>' class='images likedImage' title='Like' onclick="addToFields('<?="$counter" ?>','<?=$friendId; ?>', '<?=$friends[$counter]['name'] ?>','<?=$my_id; ?>','NULL', '<?=$count?>')" ></div>
