...
if ($token) {
	...
	$query = "SELECT friend_id from user_choices where choice = 'removed' AND uid = $my_id";
	$resource = pg_query($conn, $query);

	$removed_friends = pg_fetch_all_columns($resource, 0);
	$key = 0;
	$friends = array();
	foreach($removed_friends as $k => $value)
	{
...
?>
					...
					<div class="options">
						...
						<div id='<?php echo "remove$counter"; ?>' class='images tickImage' title="Remove" onclick="addToFields('<?="$counter" ?>','<?=$friendId; ?>', '<?=$friends[$counter]['name'] ?>','<?=$my_id; ?>','NULL', '<?=$count?>')" ></div>
